...
module.exports = {
  ...
  darkenHslBy: function (target, ratio) {
    const darkerHslArray = Color(target).darken(ratio).hsl().round().array();
    ...
    if (typeof input === "object" && input !== null) {
      Object.entries(input).forEach(([rule, value]) => {
        ...
        if (!input.hasOwnProperty("primary-focus")) {
          resultObj["--pf"] = this.darkenHslBy(input["primary"], 0.2);
        }

        if (!input.hasOwnProperty("secondary-focus")) {
          resultObj["--sf"] = this.darkenHslBy(input["secondary"], 0.2);
        }

        if (!input.hasOwnProperty("accent-focus")) {
          resultObj["--af"] = this.darkenHslBy(input["accent"], 0.2);
        }

        if (!input.hasOwnProperty("neutral-focus")) {
          resultObj["--nf"] = this.darkenHslBy(input["neutral"], 0.2);
        }
        ...
        if (!input.hasOwnProperty("base-200")) {
          resultObj["--b2"] = this.darkenHslBy(input["base-100"], 0.1);
        }

        if (!input.hasOwnProperty("base-300")) {
          if (input.hasOwnProperty("base-200")) {
            resultObj["--b3"] = this.darkenHslBy(input["base-200"], 0.1);
          } else {
  ...
  injectThemes: function (addBase, config, themes) {
    ...
    // add custom themes
    if (Array.isArray(config("ampui.themes"))) {
      config("ampui.themes").forEach((item, index) => {
    ...
    let themeOrder = [];
    if (Array.isArray(config("ampui.themes"))) {
      config("ampui.themes").forEach((theme, index) => {
    ...
    } else if (config("ampui.themes") != false) {
    ...
    themeOrder.forEach((themeName, index) => {
      if (index === 0) {
        ...
        // auto dark
        if (!config("ampui.noDarkTheme") && config("ampui.darkTheme")) {
          if (themeOrder[0] != config("ampui.darkTheme") && themeOrder.includes(config("ampui.darkTheme"))) {
            addBase({
              ["@media (prefers-color-scheme: dark)"]: {
                [":root"]: includedThemesObj[`[data-theme=${config("ampui.darkTheme")}]`],
              },
            });
          }
        } else if (!config("ampui.noDarkTheme")) {
